  ...
  async function saveItem(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (tab === 'carreras') await apiFetch(API.cursos, path, { method, body: JSON.stringify({
        ...(!editing ? { codigo: String(form.get('codigo')).trim().toUpperCase().replaceAll('_', '-') } : {}), nombre: form.get('nombre'), descripcion: form.get('descripcion') || null, duracionCiclos: Number(form.get('duracionCiclos')),
      }) });
      ...
      if (tab === 'cursos') await apiFetch(API.cursos, path, { method, body: JSON.stringify({
        carreraId: Number(form.get('carreraId')), cicloId: Number(form.get('cicloId')), ...(!editing ? { codigo: String(form.get('codigo')).trim().toUpperCase().replaceAll('_', '-') } : {}), nombre: form.get('nombre'),
      ...
      if (tab === 'aulas') await apiFetch(API.cursos, path, { method, body: JSON.stringify({
        ...(!editing ? { codigo: String(form.get('codigo')).trim().toUpperCase().replaceAll('_', '-') } : {}), nombre: form.get('nombre'), tipo: form.get('tipo'), capacidad: Number(form.get('capacidad')), ubicacion: form.get('ubicacion'),
      }) });
  ...
  return <>
    ...
    <Modal open={modalOpen} onClose={() => { setModalOpen(false); setEditing(null); }} title={`${editing ? 'Editar' : 'Crear'} ${singular[tab]}`} description={editing ? 'Los códigos de identificación se conservan al actualizar.' : 'Completa los datos obligatorios definidos por el servicio.'} wide={tab === 'cursos'}>
      <form className="entity-form" onSubmit={saveItem}><div className="form-grid">
        {tab === 'carreras' && <>{!editing && <div className="form-field"><label>Código</label><input name="codigo" required minLength={2} maxLength={20} pattern="[A-Za-z0-9-]{2,20}" title="Use solo letras, números y guion medio (-). Ejemplo: ING-SIS." onInput={(event) => { event.currentTarget.value = event.currentTarget.value.toUpperCase().replaceAll('_', '-').replace(/[^A-Z0-9-]/g, ''); }} /><small>Solo letras, números y guion medio. Ejemplo: ING-SIS.</small></div>}<div className="form-field"><label>Nombre</label><input name="nombre" defaultValue={(editing as Carrera | null)?.nombre} required maxLength={120} /></div>
        ...
        {tab === 'cursos' && <><div className="form-field"><label>Carrera</label><select name="carreraId" required defaultValue={(editing as Curso | null)?.carreraId ?? ''}><option value="" disabled>Selecciona</option>{careers.filter((item) => item.estado === 'ACTIVO' || item.id === (editing as Curso | null)?.carreraId).map((item) => <option key={item.id} value={item.id}>{item.codigo} · {item.nombre}</option>)}</select></div>
          ...
          {!editing && <div className="form-field"><label>Código</label><input name="codigo" required minLength={2} maxLength={20} pattern="[A-Za-z0-9-]{2,20}" title="Use solo letras, números y guion medio (-)." onInput={(event) => { event.currentTarget.value = event.currentTarget.value.toUpperCase().replaceAll('_', '-').replace(/[^A-Z0-9-]/g, ''); }} /><small>Ejemplo: MAT-101.</small></div>}<div className="form-field"><label>Nombre</label><input name="nombre" defaultValue={(editing as Curso | null)?.nombre} required maxLength={150} /></div>
        ...
        {tab === 'aulas' && <>{!editing && <div className="form-field"><label>Código</label><input name="codigo" required minLength={2} maxLength={20} pattern="[A-Za-z0-9-]{2,20}" title="Use solo letras, números y guion medio (-)." onInput={(event) => { event.currentTarget.value = event.currentTarget.value.toUpperCase().replaceAll('_', '-').replace(/[^A-Z0-9-]/g, ''); }} /><small>Ejemplo: AULA-201.</small></div>}<div className="form-field"><label>Nombre</label><input name="nombre" defaultValue={(editing as Aula | null)?.nombre} required maxLength={100} /></div>
