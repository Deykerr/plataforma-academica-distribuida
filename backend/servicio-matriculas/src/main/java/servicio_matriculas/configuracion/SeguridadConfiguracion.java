...
import servicio_matriculas.excepcion.ErrorApi;
        ...
        http.csrf(csrf -> csrf.disable())
                ...
                .oauth2ResourceServer(oauth2 -> oauth2
                        ...
                        .authenticationEntryPoint((request, response, ex) -> escribirError(response,
                                objectMapper, HttpStatus.UNAUTHORIZED, "Se requiere un token valido",
                                request.getRequestURI())))
                .exceptionHandling(ex -> ex
                        .authenticationEntryPoint((request, response, cause) -> escribirError(response,
                                objectMapper, HttpStatus.UNAUTHORIZED, "Se requiere autenticacion",
                                request.getRequestURI()))
                        .accessDeniedHandler((request, response, cause) -> escribirError(response,
                                objectMapper, HttpStatus.FORBIDDEN,
