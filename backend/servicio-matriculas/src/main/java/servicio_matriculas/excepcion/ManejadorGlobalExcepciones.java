...
public class ManejadorGlobalExcepciones {
    ...
    ResponseEntity<ErrorApi> manejarNoEncontrado(RecursoNoEncontradoException ex,
    ...
    ResponseEntity<ErrorApi> manejarConflicto(Exception ex, HttpServletRequest request) {
        String mensaje = ex instanceof ConflictoException ? ex.getMessage()
                : "La operacion entra en conflicto con datos existentes o fue procesada simultaneamente";
    ...
    ResponseEntity<ErrorApi> manejarDependencia(DependenciaNoDisponibleException ex,
                                                 HttpServletRequest request) {
        return respuesta(HttpStatus.SERVICE_UNAVAILABLE, ex.getMessage(), request, null);
    }

    @ExceptionHandler(AccessDeniedException.class)
    ResponseEntity<ErrorApi> manejarAcceso(AccessDeniedException ex, HttpServletRequest request) {
        return respuesta(HttpStatus.FORBIDDEN, "No tiene permisos para realizar esta operacion",
                request, null);
    }

    @ExceptionHandler(MethodArgumentNotValidException.class)
    ResponseEntity<ErrorApi> manejarValidacion(MethodArgumentNotValidException ex,
                                                HttpServletRequest request) {
    ...
    ResponseEntity<ErrorApi> manejarFormato(Exception ex, HttpServletRequest request) {
        return respuesta(HttpStatus.BAD_REQUEST, "La solicitud no tiene un formato valido", request, null);
    }

    @ExceptionHandler(Exception.class)
    ResponseEntity<ErrorApi> manejarInesperado(Exception ex, HttpServletRequest request) {
