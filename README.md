...
Están implementados los tres primeros microservicios:
...
El Servicio de Matrículas integra las APIs de Usuarios y Cursos, controla cupos concurrentes y choques de horario, y expone reportes y validación para el futuro Servicio de Evaluaciones.
...
backend/
  ...
  servicio-evaluaciones/   # Pendiente
base-de-datos/
  diagramas-er/
  scripts-sql/
documentacion/
...
Primero levante las tres bases PostgreSQL:

```powershell
cd infraestructura
docker compose up -d postgres-usuarios postgres-cursos postgres-matriculas
...
Las credenciales anteriores son solo para desarrollo local. Antes de desplegar se deben cambiar `ADMIN_PASSWORD`, `JWT_SECRET` y las contraseñas de las tres bases.
